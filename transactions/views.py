...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if user_account.balance < amount:
            messages.error(self.request, "Insufficient balance. Withdrawal not allowed.")
            return self.form_invalid(form)
...
def transfer_amount(request):
    if request.method == "POST":
        from_account = get_object_or_404(UserBankAccount, user=request.user)
        form = TransferForm(request.POST, account=from_account)
        if form.is_valid():
            ...
            from_account.save()
            to_account.save()
            messages.success(request, f'Loan request for {"{:,.2f}".format(float(amount))}$ submitted successfully')
        else:
